...
public class DemandPanelController {
                ...
                if (mouseEvent.getButton().equals(MouseButton.PRIMARY)) {
                    if (mouseEvent.getClickCount() == 2) {
                        if (isRequestValid(j)) {
                            ...
                            civilization.getDemandRequests().put(DatabaseController.getInstance().activeUser().getCivilization(), request);
                        } else {
    ...
    public void setRequestTexts() {
        int i = 0;
        for (Civilization civilization : DatabaseController.getInstance().activeUser().getCivilization().getDemandRequests().keySet()) {
            requests.add(new Label(civilization.getName() + " " + DatabaseController.getInstance().activeUser().getCivilization().getDemandRequests().get(civilization)));
            setRequestText(i, acceptAndDecline, requests, anchorPane);
            acceptAndDecline.get(i).getValue().setOnMouseClicked(mouseEvent -> {
                if (mouseEvent.getButton().equals(MouseButton.PRIMARY)) {
                    if (mouseEvent.getClickCount() == 2) {
                        DatabaseController.getInstance().demandRemoveRequest(civilization);
    ...
    public void demandCheck(String request) {
        ...
        if (parts[2].equalsIgnoreCase("gold")) {
            if (second.getGold() >= value) {
                DatabaseController.getInstance().setCivilizationGold(second,second.getGold() - value);
                DatabaseController.getInstance().setCivilizationGold(first,first.getGold() + value);
                DatabaseController.getInstance().demandRemoveRequest(first);
            ...
            if (second.getCities().size() >= value + 1) {
               ...
               //     first.addCity(city);
                    ...
                    DatabaseController.getInstance().demandRemoveCityRequest(city);

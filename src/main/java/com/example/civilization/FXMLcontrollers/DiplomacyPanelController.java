...
public class DiplomacyPanelController {
    ...
    public void setTexts() {
        ...
        for (Civilization civilization : DatabaseController.getInstance().activeUser().getCivilization().getStatusWithOtherCivilizations().keySet()) {
            ...
            if (DatabaseController.getInstance().activeUser().getCivilization().getStatusWithOtherCivilizations().get(civilization).equals(false)) {
            ...
            civilizations.get(i).setOnMouseClicked(mouseEvent -> {
                if (mouseEvent.getButton().equals(MouseButton.PRIMARY)) {
                    if (mouseEvent.getClickCount() == 2) {
                        ...
                        DatabaseController.getInstance().putStatusCivilization(civilization1,!DatabaseController.getInstance().activeUser().getCivilization().getStatusWithOtherCivilizations().get(civilization1));
                     ...
                     //   civilization1.getStatusWithOtherCivilizations().put(DatabaseController.getInstance().getDatabase().getActiveUser().getCivilization(), !civilization1.getStatusWithOtherCivilizations().get(DatabaseController.getInstance().getDatabase().getActiveUser().getCivilization()));
                        DatabaseController.getInstance().putStatusCivilization(civilization1,DatabaseController.getInstance().activeUser().getCivilization(),!civilization1.getStatusWithOtherCivilizations().get(DatabaseController.getInstance().activeUser().getCivilization()));
                        if (DatabaseController.getInstance().activeUser().getCivilization().getStatusWithOtherCivilizations().get(civilization).equals(false)) {
